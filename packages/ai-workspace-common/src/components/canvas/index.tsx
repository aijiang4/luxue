...
const Flow = memo(({ canvasId }: { canvasId: string }) => {
  ...
  const handleNodeClick = useCallback(
    (event: React.MouseEvent, node: CanvasNode<any>) => {
      ...
      if (['memo', 'skill', 'group', 'image', 'videoStand'].includes(node.type)) {
        return;
      }
  ...
  const memoizedLaunchPad = useMemo(
    () => (
      <div className="absolute bottom-[8px] left-1/2 -translate-x-1/2 w-[444px] z-[8000]">
  ...
  const { 
    ...
    drawingEraser: state.drawingEraser,
    drawingEraserSize: state.drawingEraserSize,
  }));
          ...
          {drawingMode && (
            ...
            >
              <CanvasDrawingLayer
                color={drawingColor}
                width={drawingWidth}
                eraser={drawingEraser}
                eraserSize={drawingEraserSize}
              />
            </div>
          )}
