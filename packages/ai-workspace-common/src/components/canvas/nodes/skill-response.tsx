...
import { VIDEO_STAND_EVENTS } from './video-stand';
...
export const SkillResponseNode = memo(
  ({
    ...
    // 添加一个效果，当AI提问完成后通知视频展台恢复摄像头
    useEffect(() => {
      // 只在状态从执行中变为完成或失败时触发
      if (
        (prevStatus.current === 'executing' || prevStatus.current === 'waiting') &&
        (status === 'finish' || status === 'failed')
      ) {
        // 检查是否有视频展台节点ID
        const videoStandNodeId = data?.metadata?.videoStandNodeId;
        if (videoStandNodeId) {
          console.log('AI提问完成，通知视频展台恢复摄像头:', videoStandNodeId);
          // 发送事件通知视频展台恢复摄像头
          nodeActionEmitter.emit(
            createNodeEventName(videoStandNodeId, VIDEO_STAND_EVENTS.RESTORE_CAMERA)
          );
        }
      }
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {status !== 'failed' && content && (
                    <ContentPreview
                      ...
                      maxCompactLength={100}
                      isLoading={status === 'executing' || status === 'waiting'}
                      sources={sources}
                    />
                  )}
                </div>
