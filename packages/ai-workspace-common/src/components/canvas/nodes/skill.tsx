...
export const SkillNode = memo(
  ({ data, selected, id }: NodeProps<SkillNode>) => {
    ...
    const handleSendMessage = useCallback(() => {
      ...
      // 记录源节点的entityId和type，用于后续重建连接
      const sourceNodes = sourceNodeIds.map(sourceId => {
        const sourceNode = getNodes().find(n => n.id === sourceId);
        return sourceNode ? {
          type: sourceNode.type as CanvasNodeType,
          entityId: sourceNode.data?.entityId
        } : null;
      }).filter(Boolean) as CanvasNodeFilter[];
      
      console.log('AI提问节点源节点信息:', sourceNodes);

      // 查找视频展台节点
      const videoStandNode = sourceNodes.find(node => node.type === 'videoStand');
      const videoStandNodeId = videoStandNode ? videoStandNode.entityId : null;
      ...
      setTimeout(() => {
        ...
        // 去重
        const uniqueSourceNodes = allSourceNodes.filter((node, index, self) => 
          index === self.findIndex(n => n.entityId === node.entityId && n.type === node.type)
        );
        
        console.log('创建结果节点，连接源节点:', uniqueSourceNodes);
    ...
    }, [id, getNode, getNodes, getEdges, deleteElements, invokeAction, canvasId, addNode, form]);
