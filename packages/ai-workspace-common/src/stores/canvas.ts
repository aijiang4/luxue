...
export interface CanvasState {
  ...
  setDrawingEraser: (enabled: boolean) => void;
  setDrawingEraserSize: (size: number) => void;
}
...
export const useCanvasStore = create<CanvasState>()(
  persist(
    immer((set) => ({
      ...
      setDrawingEraser: (enabled) =>
        set((state) => {
          state.drawingEraser = enabled;
        }),
      setDrawingEraserSize: (size) =>
        set((state) => {
          state.drawingEraserSize = size;
        }),
    })),
    {
      name: 'canvas-storage',
