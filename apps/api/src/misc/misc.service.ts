...
export class MiscService implements OnModuleInit {
  ...
  async savePencilImage(imageData: string, filename: string): Promise<string> {
    try {
      // 从 base64 中提取实际的图片数据
      ...
      // 使用 MinIO 存储图片
      await this.minioClient('public').putObject(storageKey, imageBuffer, {
        'Content-Type': contentType,
      });

      // 生成访问 URL
      const url = this.generateFileURL({ visibility: 'public', storageKey });
      
      this.logger.debug(`Image saved successfully at ${url}`);
      return url;
    } catch (error) {
      this.logger.error('Error saving pencil image:', error);
      throw error;
    }
  }

  async getPublicFileStream(storageKey: string): Promise<StreamableFile> {
    try {
      const data = await this.minioClient('private').getObject(storageKey);
      return new StreamableFile(data);
    } catch (error) {
      this.logger.error(`Failed to get public file stream for ${storageKey}:`, error);
      throw new NotFoundException();
    }
  }
}
